...
def scan(limit: int = 12):
    ...
    for symbol in settings.watchlist_symbols[: max(1, min(limit, 30))]:
        try:
            ...
            results.append({"symbol": symbol, "company_name": r.company_name, "score": r.overall_score, "confidence": r.overall_confidence, "verdict": r.verdict, "price": r.price, "actionable": r.entry_plan.get("actionable", False), "block_reasons": r.entry_plan.get("block_reasons", []), "tomorrow_bias": r.tomorrow_outlook.get("bias"), "market_regime": (r.data_quality.get("market_regime") or {}).get("regime")})
        except Exception as exc:
